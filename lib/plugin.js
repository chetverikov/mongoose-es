...
class MongooseES {
  ...
  sync(options) {
    ...
    stream.on('data', doc => {
      this
        ._streamDataHandler(doc, collector, stream)
        .then(changed_collector => {
          collector = changed_collector;
        })
  ...
  _streamDataHandler(doc, collector, stream) {
    return Promise
      ...
      .then(transformed => {
        collector.push([{index: {_id: doc._id.toString()}}, transformed]);
   ...
   * @private
   */
  _streamCloseHandler(collector, defer) {
